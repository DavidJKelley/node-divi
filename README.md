...
divi.auth('myusername', 'mypassword')

divi.getDifficulty(function() {
...
```js
divi.exec('getNewAddress')

divi.exec('getbalance', function(err, balance) {
...
<td> <b>Currently only available on testnet</b> Add a nrequired-to-sign multisignature address to the wallet. Each key is a divi address or hex-encoded public key. If [account] is specified, assign address to [account]. </td>
<td> N
</td></tr>
<tr>
<td> backupwallet </td>
...
<td> [account] </td>
<td> Returns the current divi address for receiving payments to this account. </td>
<td> N
</td></tr>
<tr>
...
<td> getgenerate </td>
<td> </td>
<td> Returns true or false whether divid is currently generating hashes </td>
<td> N
</td></tr>
<tr>
...
<td> getnewaddress </td>
<td> [account] </td>
<td> Returns a new divi address for receiving payments.  If [account] is specified (recommended), it is added to the address book so payments received with the address will be credited to [account]. </td>
<td> N
</td></tr>
<tr>
...
<p>To get a list of accounts on the system, execute divid listreceivedbyaddress 0 true
</p>
</td>
<td> N
</td></tr>
...
You may pass an optional function `passphrasecallback` to the `node-divi` initialization function to manage wallet unlocks. `passphrasecallback` should be a function accepting three arguments:
...
By default `divid` exposes its JSON-RPC interface via HTTP; that is, all RPC commands are transmitted in plain text across the network! To secure the JSON-RPC channel you can supply `divid` with a self-signed SSL certificate and an associated private key to enable HTTPS. For example, in your `divi.conf`:

    rpcssl=1
    rpcsslcertificatechainfile=/etc/ssl/certs/divid.crt
    rpcsslprivatekeyfile=/etc/ssl/private/divid.pem

In order to securely access an SSL encrypted JSON-RPC interface you need a copy of the self-signed certificate from the server: in this case `divid.crt`. Pass your self-signed certificate in the `ca` option and set `https: true` and node-divi is secured!
    
```js
var fs = require('fs')

var ca = fs.readFileSync('divid.crt')
